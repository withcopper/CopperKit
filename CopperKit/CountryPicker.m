          ...
          forComponent:(__unused NSInteger)component reusingView:(UIView *)view
{
    ...
    // Doug Williams - 5/12/16 fix to ensure picture show, even in a frame work :)
    NSBundle *localBundle  = [NSBundle bundleForClass:[self class]];
    NSString *rp = localBundle.resourcePath;
    NSString *bundleId = @"/CountryPicker.bundle/";
    NSString *dir = [rp stringByAppendingString:bundleId];
    NSString *file = [[self class] countryCodes][(NSUInteger) row];
    NSString *imagePath = [dir stringByAppendingString:file];
    // End FIX
